...
export class LoginComponent implements OnInit {
  ...
  protected readonly returnUrl = signal('/articles');
  ...
  protected submit(): void {
    ...
    this.authService.login(this.form.getRawValue()).subscribe({
      ...
      error: (error: unknown) => {
        this.isLoading.set(false);
        this.errorMessage.set(this.extractErrorMessage(error, 'Unable to log in right now.'));
      },
    });
  }

  protected controlError(controlName: 'email' | 'password'): string | null {
    ...
    if (control.hasError('minlength')) {
      return `${this.labelFor(controlName)} must be at least 6 characters long.`;
    }
